...
$p\le0.02$, for REMI/TSD/MTF vs.\ $0.49$--$0.78$, n.s., for ABC, which has
no velocity channel). The control makes this causal within a single model:
CLaMP-2 detects velocity under MTF but is blind to it under ABC, and the
whole profile replicates on a second corpus; microtiming and tempo stay at
or below the floor everywhere. (ii)~Across $15$ dataset pairs, the
...
at the noise floor ($0.54$, $p=0.37$). This is mechanistic: ABC notation
has no velocity channel, so flattening velocity changes nothing in the ABC
string and the embedding barely moves.\footnote{ABC does register a small
\emph{systematic} shift for some perturbations that alter note timing
(tempo and combined, permutation $p<0.05$ for \texttt{CLaMP1-ABC}), but
always \emph{below} its own sampling-noise floor ($\snr<1$) --- systematic
yet negligible, and far below the velocity response of the
velocity-bearing pipelines.}
...
unchanged --- the response vanishes ($\snr=0.67$, $p=0.21$). Conversely,
holding ABC fixed and swapping the model (\texttt{CLaMP1-ABC}
vs.\ \texttt{CLaMP2-ABC}) leaves the profile flat in both cases. The
representation, not the model, gates what the metric can detect.

\textbf{(3)~Microtiming and tempo.} $16$th-note quantization (SNR
$0.05$--$0.66$) and tempo flattening (SNR $0.06$--$0.69$) do not exceed the
...
($\snr=4.15$, $2.01$, $1.70$; all $p<0.01$) and by neither ABC pipeline
($0.49$ and $0.78$, n.s.). The replication also passes a built-in sanity
...
\addlinespace
\texttt{CLaMP1-ABC} & Velocity removed & 0.0137 & {0.49} & [0.020,\,0.030] & 0.60 \\
\texttt{CLaMP1-ABC} & Timing quantized & 0.0138 & {0.50} & [0.019,\,0.034] & 0.65 \\
\texttt{CLaMP1-ABC} & Tempo flattened & 0.0144 & {0.52} & [0.020,\,0.031] & 0.42 \\
\texttt{CLaMP1-ABC} & All combined & 0.0169 & {0.61} & [0.022,\,0.033] & 0.06 \\
\addlinespace
\texttt{CLaMP2-ABC} & Velocity removed & 0.2637 & {0.78} & [0.308,\,0.460] & 0.10 \\
\texttt{CLaMP2-ABC} & Timing quantized & 0.2257 & {0.67} & [0.274,\,0.390] & 0.25 \\
\texttt{CLaMP2-ABC} & Tempo flattened & 0.2425 & {0.72} & [0.293,\,0.402] & 0.08 \\
\texttt{CLaMP2-ABC} & All combined & 0.2356 & {0.70} & [0.291,\,0.380] & 0.16 \\
\bottomrule
\end{tabular}
\end{table}
...
the surviving attributes register. The same-model control makes the first
clause causal rather than correlational: with weights and data held fixed,
switching CLaMP-2 from MTF to ABC switches the velocity response off. The
rank-agreement result sharpens this:
...
represented (SNR up to $4.2$ across two corpora; invisible to ABC), the
same-model control shows the response switching off when only the
representation changes, microtiming and tempo stay below the noise floor
everywhere, and detail-preserving representations rank corpora alike
($\rho=0.825$, $p<0.001$) while ABC agrees with none. We recommend
reporting FMD \emph{sensitivity}, not just FMD, and provide a small,
reusable, fully scripted protocol for doing so.
